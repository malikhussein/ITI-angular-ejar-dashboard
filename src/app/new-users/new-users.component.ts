...
export class NewUsersComponent implements OnInit {
  ...
  confirmingVerifyUserId: string | null = null;
  ...
  loadUsers() {
    ...
    this.userService.getAllUsers().subscribe({
      next: (res) => {
        ...
        this.users = res.users.filter((u) => !u.isVerified);
        this.loading = false;
      },
      error: () => {
...
toggleVerification(user: User): void {
  this.confirmingVerifyUserId = user._id;
}
getVerificationText(): string {
  const user = this.users.find(u => u._id === this.confirmingVerifyUserId);
  return user?.isVerified ? 'unverify' : 'verify';
}


cancelToggle(): void {
  this.confirmingVerifyUserId = null;
}

confirmToggleVerification(user: User): void {
  if (user._id === this.currentAdminId) {
    this.showToastMessage("You cannot change your own verification status. Please contact another admin.", 'error');
    this.confirmingVerifyUserId = null;
    return;
  }
  ...
  this.userService.toggleVerification(user._id).subscribe({
    ...
    error: () => {
      this.showToastMessage('Failed to toggle verification', 'error');
      this.confirmingVerifyUserId = null;
    }
  });
}
...
confirmToggleVerificationById(id: string): void {
  const user = this.users.find(u => u._id === id);
  if (user) {
    this.confirmToggleVerification(user);
  }
}
